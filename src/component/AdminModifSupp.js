...
function AdminModifSupp() {
	...
	const deleteAnnonce = (e) => {
		axios
			.post(config.SERVER + `/annonces/deleteannonce`, { id: e.target.name })
			.then((res) => {
				getAllUserAnnonce();
			})
			.catch((erreur) => {
	...
	const getAllUserAnnonce = () => {
		...
		axios
			.get(config.SERVER + "/annonces/readannonce")
			.then((res) => {
	...
	const afficheButtonAddAnnonce = () => {
		return (
			<div
				className="buttonAddAnnonce"
				style={{ maxWidth: "400px", marginLeft: "auto", marginRight: "auto" }}
			>
				{" "}
				<h3 style={{ color: "#ee3f89" }}>
					{" "}
					Aucune annonce disponible sur votre site
				</h3>
			</div>
		);
	};
	...
	const handlestatutChange = (e, data) => {
		...
		axios
			.post(config.SERVER + `/annonces/update`, temps)
			.then((res) => {
	...
	const tabpaneAfficheEnattente = () => {
		return data.map((item, index) => {
			if (item.statut == "En attente") {
				return (
					<Card className="list_item_container_modif_supp" key={index}>
						...
						<div className="div_contenant_texte_annonce">
							<CardBody>
								...
								<div className="buttonModifSupprim">
									...
									<Link to={"/updateannonce/" + item._id}>
										<Button className="modif">Modifier</Button>
									</Link>
									<Button
										className="suppression_detail"
										name={item._id}
	...
	return (
		<div className="div_item_annonce">
			<Button color="primary" onClick={handleDeconnexion}>
				Se déconnecter
			</Button>
			...
			{data.length > 0 ? (
				<TabContent activeTab={activeTab}>
					<TabPane tabId="1"> {tabpaneAfficheEnligne()}</TabPane>
					<TabPane tabId="2">{tabpaneAfficheEnattente()}</TabPane>
					<TabPane tabId="3">{tabpaneAfficheSupprime()}</TabPane>{" "}
				</TabContent>
			) : (
